...
policies. The problem therefore cannot assume one global feature set or one
global metric list that is equally relevant to all task kinds.
...
rare contexts, or recently changed workloads.
...
needed by downstream control layers.
...
critical-path or retry-expansion semantics.
...
related signals or treat symptoms as independent causes.
...
The profiling problem must preserve enough structure to combine component
...
relevant state is observable.

The eighth challenge is profiling overhead. Instrumentation is not free. A
profiling model must decide not only what would be useful to observe, but what is
worth observing under an overhead budget.

\subsection{Scope and Assumptions}
\label{sec:problem-scope-assumptions}

This paper assumes that task attempts can be identified at least by task kind
and attempt boundary. It assumes that some task metadata or intrinsic features
may be available before or during execution, that execution-context information
may be available but incomplete, and that profiling decisions are useful even
when final scheduling, routing, placement, or admission decisions are made by
another layer.
...
workload metrics, nor does it require exact prediction for arbitrary tasks. The
goal is to produce structured estimates, expose uncertainty, and guide profiling
policy.
